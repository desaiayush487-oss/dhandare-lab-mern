...
import { Droplet, Activity, AlertCircle, CheckCircle, Loader2, ShieldCheck } from 'lucide-react';

const DiabetesPrediction = () => {
  const navigate = useNavigate();
  
  // Exactly 4 fields matching the Python script
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ⚠️ STRICT ORDER: [Glucose, BP, BMI, Age]
      const payload = {
        ...
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-gray-100">
          ...
          <div className="w-full md:w-2/3 p-8 lg:p-12">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-3xl font-bold text-gray-800">Diabetes Risk AI</h2>
            </div>
            <p className="text-gray-500 mb-8 ml-1">Enter patient vitals to generate an AI-powered glycemic risk analysis.</p>

            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Glucose Level</label>
                <input type="number" name="glucose" value={formData.glucose} onChange={handleChange} required
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all bg-gray-50 focus:bg-white" />
              </div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">BMI</label>
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              {prediction === null ? (
                ...
                <div className="animate-fade-in">
                  ...
                  <p className="text-slate-300 text-lg">The AI model indicates a high probability of Diabetes. Consult an endocrinologist.</p>
                </div>
              ) : (
                <div className="animate-fade-in">
                  ...
                  <p className="text-slate-300 text-lg">The AI model indicates normal glycemic health based on current vitals.</p>
                </div>
              )}
            </div>
          </div>
